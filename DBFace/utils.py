...
def drawbbox(image, bbox, color=None, thickness=2, textcolor=(0, 0, 0), landmarkcolor=(0, 0, 255)):
    ...
    border = thickness / 2
    pos = (x + 3, y - 5)
    cv2.rectangle(image, intv(x - border, y - 21,
                  w + thickness, 21), color, -1, 16)
...
def detect(model, image, threshold=0.4, nms_iou=0.5, cuda=True):
